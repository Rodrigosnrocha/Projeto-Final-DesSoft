...
def load_assets():
    ...
    assets[IMG_PLAYER_TEST] = pygame.image.load(path.join(IMG,'aviao2.png')).convert_alpha() # Adiciona a imagem do avião do jogador
    assets[IMG_PLAYER_TEST] = pygame.transform.scale(assets[IMG_PLAYER_TEST], (194, 110)) # Define o tamanho do avião do jogador
    assets[IMG_BULLET_TEST] = pygame.image.load(path.join(IMG,'bullet.png')).convert_alpha() # Adiciona a imagem do "tiro"
    assets[IMG_BULLET_TEST] = pygame.transform.scale(assets[IMG_BULLET_TEST], (40, 12)) # Define o tamanho do "tiro"
    assets[IMG_ENEMY_TEST] = pygame.image.load(path.join(IMG,'aviao3.png')).convert_alpha() # Adiciona a imagem do avião inimigo
    assets[IMG_ENEMY_TEST] = pygame.transform.scale(assets[IMG_ENEMY_TEST], (118, 42)) # Define o tamanho do avião inimigo
    ...
    assets['shoot_sound'] = pygame.mixer.Sound(path.join(SND,'ES_Gunshot Shotgun 159.mp3'))
    coin_anim = []
    for i in range(6):
